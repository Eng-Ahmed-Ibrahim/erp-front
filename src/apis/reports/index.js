...
export async function getAllinvoicesOutGoing(filteredValues, id, setIsLoading) {
  try {
    ...
    const res = await axios.get(
      `${domain}/api/v1/store/invoice/filter/get_recipes/out_going_from_department_from_to_date/${id}`,
      {
        params: {
          from: from_date,
          to: to_date,
...
export async function getTotalForSupplierReports(
  filteredValues,
  id,
  setIsLoading
) {
  try {
    ...
    const res = await axios.get(
      `${domain}/api/v1/store/invoice/filter/get_recipes/supplier_from_to_date/${id}`,
      {
        params: {
          "date[from]": from_date,
          "date[to]": to_date,
...
export async function getReportOfRecipe(
  filteredValues,
  id,
  setIsLoading,
  department_id
  ...
  try {
    setIsLoading(true);
    const { name, page, from_date, to_date, invoice_type } = filteredValues;
    const default_from = "1970-01-01";
    const default_to = new Date().toISOString().split("T")[0];

    const res = await axios.get(
      `${domain}/api/v1/store/recipe/recipe-invoices-report/statistics/${id}`,
      {
        params: {
...
export async function getSupplierInvoicesReport(
  filteredValues,
  id,
  setIsLoading
) {
  try {
    ...
    const res = await axios.get(
      `${domain}/api/v1/store/invoice/suppliers/invoices`,
      {
        params: {
          "date[from]": from_date,
          "date[to]": to_date,
