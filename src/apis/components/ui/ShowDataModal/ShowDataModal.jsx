...
const ShowDataModal = ({
  ...
  useEffect(() => {
    ...
    console.log(`ssdataaa`, initialEditedData);
    // if (responseData.recipes) {
    //   console.log("sjiij")
    //   responseData.recipes.forEach((recipe, index) => {
    //     console.log(`Recipe ${index + 1} unit:`, recipe.unit ? recipe.unit.name : "No unit available");
    //   });
    // }
  ...
  const renderInputField = (header, value, index, subKey) => {
    console.log(`idi`, header, value, index, subKey)
  ...
  console.log(`nonArrayHeaders, arrayHeaders`, nonArrayHeaders, arrayHeaders);
  return (
    <div className="show-data-modal">
      <div className="modal-content">
                  ...
                  {nonArrayHeaders.map((header, index) => (
                    <td key={index}>
                      {header.isInput
                        ...
                        : header.key === "quantity" // Check if the key is "quantity"
                          ? `${responseData[header.key]} ${responseData.unit?.name || ''}` // Display the quantity with unit
                          : responseData[header.key] // Display the actual value for other headers
                      }
                    </td>
                  ))}
                </tr>
                    ...
                    {responseData[header.key]?.map((item, itemIndex) => (
                      <tr key={itemIndex}>
                        {header.details.map((detail, detailIndex) => (
                          <td key={detail.key}>
                            {detail.key === "quantity" && item["unit"]
                              ? `${item[detail.key]} ${item["unit"].name || ''}`
                              : detail.isInput
                                ? renderInputField(
                                  header.key,
                                  item[detail.key],
        ...
        {shouldPrint && <PrintAfterSubmit id={responseData.id} tableNO={responseData.table_number} />} {/* Conditional rendering */}
      </div>
