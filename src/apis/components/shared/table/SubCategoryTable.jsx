...
const SubCategoryTable = ({
  ...
  actions,
  id,
  filterValues, // Accept filterValues from props
  setFilterValues
}) => {
  ...
  const [debouncedValue] = useDebounce(filterValues, 1000)
  ...
  //     }
  //   );
  // }, [filterValues, currentPage]); // Include filterValues in dependency array

  useEffect(() => {
    // Fetch data using filterValues
    fetchData({ ...debouncedValue, page: currentPage }, id, setIsLoading).then(
      (result) => {
        if (result && result.data) {
          setData(result);
        } else {
  ...
  }, [debouncedValue, currentPage]); // Include filterValues in dependency array
    ...
    if (type === "number") {
      ...
      return (
        <input
          ...
          value={filterValues[key] || ""}
          onChange={(e) => {
            filter?.handleInputChange ? filter.handleInputChange(e.target.value) : null
