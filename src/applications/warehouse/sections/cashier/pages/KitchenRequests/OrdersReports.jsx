...
const OrdersReports = () => {
                ...
                if (user.department.type === "reciver") {
                    ...
                    if (!isFromDateToday || dayDiff !== 1) {
                      const modal = Modal.error({
                        title: 'error',
                        content: <div style={{ fontSize: '24px', textAlign: 'center' }}>يرجى ملئ جميع البيانات بشكل صحيح. يجب أن تكون الفترة بين التاريخين يوم واحد</div>,
                        centered: true, 
                        width: 400, 
                      });
  ...
  const handlePaymentMethodChange = async (value) => {
    const selectedPaymentMethod = paymentMethods.find(method => method.id === value);
    const PaymentMethodName = selectedPaymentMethod ? selectedPaymentMethod.name : "";
...
{user.department.type === "master" ? (
  ...
  <div >
    <div className="col">
      <div className="mb-3">
        <label htmlFor="payment-method" className="form-label">طريقة الدفع</label>
        <select
          id="payment-method"
          required
          ...
          {clients.map((client) => (
            ...
            <table className="table table-hover mt-5 mb-20">
                ...
                <tbody>
                    <tr>
                        <td >
                            {Math.round(data?.data?.totals.total_visa * 100) / 100}
            ...
            <table className="table table-hover mt-5">
                <thead>
                    ...
                    const rowStyle = order.status === 'returned' 
                    ? { background: '#b9aeae' } 
                    : {};
                    const buttonStyle = order.status === 'returned' 
                    ? { width: "100%", marginTop: `27%`, transition: `all 0.3s`,background:"#444444",color:"white" } 
                    : { width: "100%", marginTop: `27%`, transition: `all 0.3s`,background:"red",color:"white" };
                    const buttonName = order.status === 'returned' ? 'تم الحذف' : 'حذف الاوردر';

                    return (
                    <React.Fragment key={index}>
                        
                    <tr style={rowStyle}>
                    <th style={rowStyle} scope="row" >{index + 1}</th>
                    <td style={rowStyle}>{order.code}</td>
                    <td style={rowStyle}>{order.status}</td>
                    <td style={rowStyle}>{order.order_date}</td>
                    <td style={rowStyle}>{order.client == "" ? "Guest" : order.client}</td>
                    <td style={rowStyle}>{Math.round(order.total_price * 100) / 100}</td>
                    <td style={rowStyle}>{order.client_type}</td>
                    <td style={rowStyle}>
                    {order.products?.map((product, index) => (
                    <li key={index}>
                    {isAdmin &&
                    `${product.name} - ${product.quantity} × ${product.price} = ${Math.round(product.total_price * 100) / 100}`
                    }
                    </li>
                    ))}
                    </td>
                    {(isAdmin&&!isTalaat)&&
                    <td style={rowStyle}><button className="form-cashier-btn"
                    onClick={()=>changeOrderStatus(order.id, "returned")}
                    style={buttonStyle}>{buttonName}</button>
                    </td>}
