...
const BeneficiariesTable = ({ selectedOfficer, onSelectOfficer }) => {
  ...
  const canEditBeneficiary = hasPermission(user, 'edit membership card beneficiary');
  const canDeleteBeneficiary = hasPermission(user, 'delete membership card beneficiary');
  ...
  const handleDelete = async (id) => {
    if (!canDeleteBeneficiary) {
      setError('ليس لديك صلاحية لحذف المستفيدين');
      return;
    }
  ...
  const handleEdit = (beneficiary) => {
    if (!canEditBeneficiary) {
      setError('ليس لديك صلاحية لتعديل المستفيدين');
      return;
    }
            ...
            {beneficiaries.length === 0 ? (
              ...
              beneficiaries.map((beneficiary, index) => (
                <tr key={beneficiary.id}>
                  ...
                  <td className="notes-cell">{beneficiary.notes || '-'}</td>
