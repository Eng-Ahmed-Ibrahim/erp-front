...
const CahierWearhouseDetailes = ({ onAddItem }) => {
  ...
  const [recipeCategoryParentsproduct, setRecipeCategoryParentsproduct] =
    useState([]);
  ...
  useEffect(() => {
    ...
    ); // Filter out any falsy values (e.g., `false` or `undefined` from the conditional fields)
  }, [
    isAddProduct,
    recipeCategoryParents,
    recipeCategories,
    recipes,
    recipeCategoryParentsproduct,
    recipeCategoriesproduct,
    recipesproduct,
  ]);
  ...
  const handleAddItem = async () => {
    let newItems = [];
console.log([selectedRecipeproduct,selectedRecipe])
    
if (selectedRecipeproduct) {
       try {

        const res = await fetch(
          `${API_ENDPOINT}/api/v1/store/products/${selectedRecipeproduct}`,
          {
            headers: {
              Authorization: `Bearer ${Token}`,
            },
        ...
        if (data.data.recipes.length == 0 ){
            message.error('هذا المنتج لا يحتوي على مكونات داخلية ')
        }else {
            setRecipesinproduct(data.data.recipes);

            data.data.recipes.forEach((recipenow) => {
              const newItem = {
                name: recipenow.name,
            ...
            console.log('here fuck u moter fukler',newItems )
        }
  ...
  return (
    <div>
      ...
      <input className="form-input" type="text" value={unit} disabled />
