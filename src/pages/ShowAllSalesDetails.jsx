...
function DataModal({ show, onHide ,discountDetils ,fromDate,
    toDate,
    ...
    >
      ...
      <button onClick={handleSavePDF} className="pdf-button"> حفظ PDF</button>
    </div>
...
{discountDetils && discountDetils.length > 0 ? (
  discountDetils.map((order, index) => (
    <tr key={order.id}>
      ...
      <td>{order.client_name ==""?"لا يوجد" :order.client_name}</td>
...
setSelectedDepartmentName(selectedDeptNames);
     try {
        const response = await axios.get(`${API_ENDPOINT}/api/v1/payment_report/`, {
            params:{
          data: { from:fromDate,
            to:toDate,
...
const handleClick = async () => {
    ...
    return (
            <div>
            <h1 style={{ display: "flex", flexDirection:"column", justifyContent: "center", alignItems: "center" }}>تقرير الايرادات المفصل</h1>
            <div style={{ display: "flex", flexDirection:"column", justifyContent: "start", alignItems: "start" }}>
            <button onClick={handleSavePDF} className="pdf-button"> حفظ PDF</button>
            ...
            <label htmlFor="exampleFormControlInput1" className="form-label ps-3 ">الي</label>
            <input
            onChange={(e) => {
            const selectedDay = e.target.value;
            ...
            style={{ width: "100%", transition: `all 0.3s` }}>تأكيد</button>
            </div>
            </div>
            <table className="table table-hover mt-5"  ref={tableRef}>
            <thead>
            ...
            <tr >
            <th >
            {Math.round(returnedData?.total_cost_price * 100) / 100}</th>
            <th  scope="row" 
            style={{ cursor: 'pointer' }} 
            onClick={handleClick} >
                {Math.round(Math.abs(returnedData?.total_discount * 100)) / 100}</th>
            <th scope="row" >{Math.round(Math.abs(returnedData?.total_tax * 100)) / 100}</th>
            <th scope="row" >{Math.round(Math.abs(returnedData?.total_price * 100)) / 100}</th>
